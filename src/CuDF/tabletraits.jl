...
function TableTraits.getiterator(df::DataFrame)
    ...
    column_data = [eltype(df[i]) == String ? [df[i][j] for j in 1:length(df)] :
                   values(df[i]) for i in col_names_raw]
...
function TableTraits.get_columns_copy_using_missing(df::CuDF.DataFrame)
    # return a named tuple of columns here
    col_names_raw = [i for i in CuDF.columns(df)]
    col_names = Symbol.(col_names_raw)
    cols = (Array(eltype(df[i]) == String ? [df[i][j] for j in 1:length(df)] : df[i]) for i in
                                                                                          col_names_raw)
